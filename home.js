...
const GAMES = [
  ...
  },
  {
    ...
    emoji: '🖌️',
    name: '一笔画',
    desc: '点阵里藏着一幅画的所有线段：挑对起点、不抬笔不重描，每条线恰好描一遍。',
    tags: ['益智', '图形', '空间'],
    href: 'yibihua/index.html',
    prefix: 'yi.',
    records: () => [scoreRec('yi.best', '分'), unitRec('yi.lines', '累计描线', '条')],
    totals: () => [num('yi.best')],
  },
  {
    id: 'fenjian',
    emoji: '🏭',
    name: '分拣工厂',
    desc: '流水线上每格都是一个箭头：改不了路就改方向，一把扳手转一格，把每种包裹送进对应货架。',
    tags: ['策略', '塔防', '休闲'],
    href: 'fenjian/index.html',
    prefix: 'fj.',
    records: () => [scoreRec('fj.best', '分'), unitRec('fj.sort', '累计入库', '件')],
    totals: () => [num('fj.best')],
  },
  {
    id: 'yingan',
    emoji: '🎧',
    name: '绝对音感',
    desc: '辨音程、跟弹旋律、听和弦性质三种轮次轮换：耳力是唯一资源，答错扣分但退还一次回放。',
    tags: ['音乐', '记忆'],
    href: 'yingan/index.html',
    prefix: 'yin.',
    records: () => [scoreRec('yin.best', '分'), unitRec('yin.right', '累计听对', '题')],
    totals: () => [num('yin.best')],
  },
  {
    id: 'diaoyu',
    emoji: '🎣',
    name: '钓鱼佬',
    desc: '黑漂那一下拼反应，中鱼之后拼遛法：按住收线张力就涨，它猛窜时必须松手，遛到没劲才能起鱼。',
    tags: ['休闲', '反应', '收集'],
    href: 'diaoyu/index.html',
    prefix: 'dy.',
    records: () => [scoreRec('dy.best', '分'), unitRec('dy.catch', '累计起鱼', '条')],
    totals: () => [num('dy.best')],
  },
  {
    id: 'domino',
    emoji: '🂡',
    name: '多米诺',
    desc: '自己一张一张摆骨牌，推倒第一张之后一切交给物理：间距、朝向、石头与弹板决定连锁能不能传到皇冠。',
    tags: ['物理', '休闲', '益智'],
    href: 'domino/index.html',
    prefix: 'dm.',
    records: () => [scoreRec('dm.best', '分'), unitRec('dm.fall', '累计倒牌', '张')],
    totals: () => [num('dm.best')],
  },
  {
    id: 'chongshi',
    emoji: '🐛',
    name: '虫食算',
    desc: '竖式被虫蛀掉了几个数字：按位往回推、进位借位都算账，但填错一次它就当场再吃掉一格你原本看得见的数。',
    tags: ['数字', '益智'],
    href: 'chongshi/index.html',
    prefix: 'cs.',
    records: () => [scoreRec('cs.best', '分'), unitRec('cs.solved', '累计咬回', '道')],
    totals: () => [num('cs.best')],
  },
  {
    id: 'parking',
    emoji: '🚗',
    name: '移个车先',
    desc: '六乘六的停车场，每辆车只能沿车头那根轴滑，滑多远都算一次。关卡是从出口倒着推出来的，最少挪几次当场用 BFS 算给你看。',
    tags: ['益智', '策略', '图形'],
    href: 'parking/index.html',
    prefix: 'pk.',
    records: () => [scoreRec('pk.best', '分'), unitRec('pk.out', '累计出库', '辆')],
    totals: () => [num('pk.best')],
  },
  {
    id: 'mohei',
    emoji: '🕵️',
    name: '摸黑',
    desc: '回合制潜行：你走一步守卫才走一步。光锥照到的格子不能站，捡齐 💎 再背起 💛 摸到 🚪，手里还有几颗石子可以捏给守卫听。',
    tags: ['策略', '益智', '潜行'],
    href: 'mohei/index.html',
    prefix: 'mh.',
    records: () => [scoreRec('mh.best', '分'), unitRec('mh.save', '累计救出', '人')],
    totals: () => [num('mh.best')],
  },
  {
    id: 'mianguan',
    emoji: '🍜',
    name: '面馆高峰期',
    desc: '一口锅一口炒锅：下锅 → 加浇头 → 上桌，三道手一道不能省。煮好了赖在锅里会坨，客人等超时掀桌走人，信誉扣光就关张。',
    tags: ['休闲', '解压', '策略'],
    href: 'mianguan/index.html',
    prefix: 'mg.',
    records: () => [scoreRec('mg.best', '分'), unitRec('mg.serve', '累计出餐', '碗')],
    totals: () => [num('mg.best')],
  },
  {
    id: 'guji',
    emoji: '🥁',
    name: '鼓机十六步',
    desc: '先在 4 条声部 × 16 步的网格上写一段律动，再亲手把它敲出来：节拍器只留四下 clicks，PERFECT / GOOD / MISS 全看你的手。写得越密越值钱，段与段之间还提速。',
    tags: ['音乐', '构筑', '反应'],
    href: 'guji/index.html',
    prefix: 'gj.',
    records: () => [scoreRec('gj.best', '分'), unitRec('gj.bars', '累计打完', '段')],
    totals: () => [num('gj.best')],
  },
  {
    id: 'sokoban',
    emoji: '📦',
    name: '推箱子',
    desc: '箱子只能推不能拉，走错一步就得整局倒退。关卡是从「已经码好」倒着拉乱生成的，保证解得开；「最优步」是现场 BFS 算出来的真最短，走得越贴它分越高，走完富余步数就要扣一次加班额度。',
    tags: ['益智', '空间'],
    href: 'sokoban/index.html',
    prefix: 'sb.',
    records: () => [levelRec('sb.lv', '最远到第'), scoreRec('sb.best', '分'), unitRec('sb.push', '累计推箱', '次')],
    totals: () => [num('sb.best')],
  },
  {
    id: 'liankan',
    emoji: '🔗',
    name: '连连看',
    desc: '两张同花色的牌之间拐弯不超过两次就连得上，路线当场画给你看，还按拐弯次数给分。牌面从成对倒放生成所以一定清得完，连着消会叠连击，卡住了有提示，实在没得连就洗牌。',
    tags: ['益智', '消除'],
    href: 'liankan/index.html',
    prefix: 'lk.',
    records: () => [levelRec('lk.lv', '最远到第'), scoreRec('lk.best', '分'), unitRec('lk.pair', '累计配对', '对')],
    totals: () => [num('lk.best')],
  },
  {
    id: 'breakout',
    emoji: '🧱',
    name: '打砖块',
    desc: '球打在挡板的哪一段就往哪边飞：边缘够斜、中间够直，落点全靠自己调。连锁不断的砖越敲越值钱，硬砖要敲好几下才碎，掉下来的水泡接住能分裂球、加宽板、放慢 tempo，漏一颗球就收场。',
    tags: ['经典', '反应'],
    href: 'breakout/index.html',
    prefix: 'bo.',
    records: () => [levelRec('bo.lv', '最远到第'), scoreRec('bo.best', '分'), unitRec('bo.brk', '累计拆砖', '块')],
    totals: () => [num('bo.best')],
  },
  {
    id: 'guandao',
    emoji: '🔧',
    name: '接根水管',
    desc: '水已经在流，你边漏边接：一格一格拧接头把水路续上，🔒 是开局就对好的固定接头，空位里的假管子别理。每处漏水都在偷你的时间，全部拧回正位才能零漏水拿满奖励。',
    tags: ['益智', '空间'],
    href: 'guandao/index.html',
    prefix: 'gd.',
    records: () => [levelRec('gd.lv', '最远到第'), scoreRec('gd.best', '分'), unitRec('gd.flow', '累计通水', '格')],
    totals: () => [num('gd.best')],
  },
  {
    id: 'tiaoyi',
    emoji: '🦘',
    name: '跳一跳',
    desc: '按住蓄力松手起跳，力度决定抛物线：落在台面正中吃 PERFECT，连续准点叠连击倍率，一脚踏空就清零。站桩不动会超时判负，跳太高撞上台沿一样扑空。',
    tags: ['休闲', '物理', '反应'],
    href: 'tiaoyi/index.html',
    prefix: 'ty.',
    records: () => [levelRec('ty.lv', '最远到第'), scoreRec('ty.best', '分'), unitRec('ty.jump', '累计起跳', '次')],
    totals: () => [num('ty.best')],
  },
  {
    id: 'klotski',
    emoji: '🏮',
    name: '华容道',
    desc: '二十格木匣只剩两个空位：木块只能整格横竖滑，不能旋转、不能抬手跨过、更不能叠。每关招牌上的「最优」是 BFS 当场量出来的真最短，走得越贴它分越高 —— 收官是老祖宗的原题横刀立马。',
    tags: ['益智', '空间'],
    href: 'klotski/index.html',
    prefix: 'kl.',
    records: () => [levelRec('kl.lv', '最远到第'), scoreRec('kl.best', '分'), unitRec('kl.step', '累计挪步', '步')],
    totals: () => [num('kl.best')],
  },

  {
    ...
    emoji: '🧮',
    name: '24 点速算',
    desc: '四张牌三个运算符名额：每次挑两张配一个 ＋－×÷ 合成一张，合到剩最后一张正好等于目标才算过。中间值可以是分数（8÷(3−8/3) 就靠它），合出一手永远到不了目标的牌会被当场判「算死了」。',
    tags: ['数字', '益智', '反应'],
    href: '24dian/index.html',
    prefix: 'pt.',
    records: () => [levelRec('pt.lv', '最远到第'), scoreRec('pt.best', '分'), unitRec('pt.solved', '累计答对', '题')],
    totals: () => [num('pt.best')],
  },

  {
    ...
    emoji: '⭕',
    name: '数圈',
    desc: '每格数字说的是「周围 8 格里有几格在圈上」：点格子把它画进圈、或打个 ✕，圈上的格必须连成一条闭合回路 —— 每格恰好两个上下左右的圈邻居，不能断、不能分叉、不能画成两坨。八关从 4×4 画到 8×8，题目现造现验：删到只剩一个解才发。',
    tags: ['益智', '图形'],
    href: 'shuquan/index.html',
    prefix: 'sq.',
    records: () => [levelRec('sq.lv', '最远到第'), scoreRec('sq.best', '分'), unitRec('sq.loop', '累计圈格', '格')],
    totals: () => [num('sq.best')],
  },
